    ...
    return (
        ...
        <section className="px-20 py-10 bg-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-6">
                {pic.map((item) => (
                    <div key={item.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:scale-105 transition-transform duration-300">
                        <img src={item.pic} alt={item.country} className="w-full h-48 object-cover"/>
                        <div className="p-4">
                            <h3 className="text-lg font-semibold">{item.country}</h3>
                            <p className="text-yellow-500 font-bold mt-2">{item.price}</p>
                            <button className="mt-4 w-full bg-[#04356A] text-white py-2 rounded hover:bg-[#032B56] transition-colors duration-300">
                                BOOK NOW
                            </button>
                        </div>
                    </div>
